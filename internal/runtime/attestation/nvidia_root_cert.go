package attestation

// NVIDIA device identity root certificate used by the NVIDIA nvtrust verifier
// when normalizing GPU attestation certificate chains.
const nvidiaDeviceRootCertificatePEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
